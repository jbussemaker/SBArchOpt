...
try:
    from open_turb_arch.architecting.architecting_problem import (
        get_architecting_problem,
        load_pareto_front,
    )
    from open_turb_arch.architecting.problem import ArchitectingProblem
    from open_turb_arch.architecting.opt_defs import *
    from open_turb_arch.tests.examples.simple_problem import (
        get_architecting_problem as get_simple_architecting_problem,
    )
...
__all__ = [
    "HAS_OPEN_TURB_ARCH",
    "SimpleTurbofanArch",
    "RealisticTurbofanArch",
    "SimpleTurbofanArchModel",
]
...
class OpenTurbArchProblemWrapper(HierarchyProblemBase):
    ...
    def __init__(self, open_turb_arch_problem: "ArchitectingProblem", n_parallel=None):
        ...
        self._obj_factors = np.array(
            [
                -1 if obj.dir == ObjectiveDirection.MAXIMIZE else 1
                for obj in open_turb_arch_problem.opt_objectives
            ]
        )
        self._con_factors = np.array(
            [
                -1 if con.dir == ConstraintDirection.GREATER_EQUAL_THAN else 1
                for con in open_turb_arch_problem.opt_constraints
            ]
        )
        self._con_offsets = np.array(
            [con.limit_value for con in open_turb_arch_problem.opt_constraints]
        )
        ...
        **kwargs,
        ...
        if self.n_parallel is None:
            ...
            with concurrent.futures.ProcessPoolExecutor(
                ...
                futures = [
                    executor.submit(self._arch_evaluate_x, x[i, :])
    ...
    def _gen_all_discrete_x(self) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        ...
        if os.path.exists(x_all_path):
            ...
            if len(data) == 2:
                ...
                if (
                    isinstance(x_all, np.ndarray)
                    and isinstance(is_act_all, np.ndarray)
                    and x_all.shape == is_act_all.shape
                    and x_all.shape[0] == self._get_n_valid_discrete()
                ):
    ...
    def _g_from_x(self):
        ...
        n_eval_g = len(
            [
                con
                for metric in self._problem.constraints
    ...
    def _choice_g_props(self):
        return [
            (
                choice,
                choice.get_constraints() is not None,
                len(choice.get_design_variables()),
            )
    ...
    def _check_pf_models(self):
        ...
        n = x.shape[0]
        is_failed = self.get_failed_points({"F": f, "G": g})
        is_failed_pred = self._models["fails"].predict(x) > 0.5
        false_pos = np.where(is_failed_pred & ~is_failed)[0]
        false_neg = np.where(~is_failed_pred & is_failed)[0]
        print(
            f"Failure prediction (n = {n}): false pos {len(false_pos)} ({100*len(false_pos)/n:.1f}%); "
        ...
        if f_pf.shape[1] == 1:
            ...
            try:
                ...
                print(
                    f"Surrogate model best (fail): {f_pf_surrogate[0]} @ {x_pf_surrogate}"
                )
        else:
            plt.figure(), plt.title(
                f"Pareto Front comparison: {self.__class__.__name__}"
            )
    ...
    def _arch_evaluate_x_surrogate(self, x: np.ndarray):
        ...
        is_failed: np.ndarray = fails > 0.5
        f[is_failed, :] = np.nan
        g[is_failed, :] = np.nan
        ...
        if np.any(g_from_x):
            for i in range(x.shape[0]):
                if is_ok[i]:
                    architecture, x_imp_ = self._problem.generate_architecture(
                        self._convert_x(x_imp[i, :])
                    )
                    ...
                    for choice, has_g, n_dv in self._choice_g_props:
                        ...
                        if has_g:
                            choice_g = choice.evaluate_constraints(
                                architecture,
                                choice_dv,
                                self._problem.analysis_problem,
                                {},
                            )
            ...
            g[:, g_from_x] = (
                g[:, g_from_x] - self._con_offsets[g_from_x]
            ) * self._con_factors[g_from_x]
    ...
    def _get_data_path(cls, sub_path: str) -> str:
        path = os.path.join(
            os.path.dirname(__file__), cls._data_folder, cls._sub_folder
        )
...
class SimpleTurbofanArch(OpenTurbArchProblemWrapper):
    ...
    def get_failure_rate(self) -> float:
        return 0.51  # Paper section IV.B
...
class RealisticTurbofanArch(OpenTurbArchProblemWrapper):
    ...
    def load_pareto_front(self):
        if self._f_pf is None:
            ...
            self._x_pf, self._f_pf, self._g_pf = self._correct_pf(
                *super().load_pareto_front()
            )
    ...
    def get_failure_rate(self) -> float:
        return 0.67  # Paper section IV.C
...
class SimpleTurbofanArchModel(SimpleTurbofanArch):
    ...
    def _arch_evaluate(
        ...
        **kwargs,
        ...
        x[:, :], f_out[:, :], g_out[:, :], is_active_out[:, :] = (
            self._arch_evaluate_x_surrogate(x)
        )
