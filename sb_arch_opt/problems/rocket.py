...
class RocketArch(HierarchyProblemBase):
    ...
    def __init__(self):
        ...
        des_vars = [
            ...
            ),  # Engine type stage 1 [VULCAIN, RS68, S_IVB, SRB, P80, GEM60] [1]
            Integer(bounds=(1, 3)),  # Nr of engines stage 1
            Choice(
                options=list(range(6))
            ),  # Engine type stage 2 [VULCAIN, RS68, S_IVB, SRB, P80, GEM60]
            Integer(bounds=(1, 3)),  # Nr of engines stage 2
            Choice(
                options=list(range(6))
            ),  # Engine type stage 3 [VULCAIN, RS68, S_IVB, SRB, P80, GEM60]
            Integer(bounds=(1, 3)),  # Nr of engines stage 3
            Real(bounds=(0, 1) if lc else (0, 45)),  # Stage 1 length [m] [7]
            Real(bounds=(0.1, 0.6) if lc else (0, 45)),  # Stage 2 length [m]
            Real(bounds=(0.2, 1) if lc else (0, 45)),  # Stage 3 length [m]
            Real(
                bounds=(10, 11) if lc else (10, 20)
            ),  # Overall length-to-diameter ratio [10]
            Choice(
                options=list(range(3))
            ),  # Head shape [Cone, Elliptical, Semi-sphere] [11]
            Real(bounds=(28, 32) if lc else (15, 45)),  # Cone angle
            Real(bounds=(0.15, 0.21) if lc else (0.1, 0.25)),  # Ellipse length ratio
        ]
    ...
    def _arch_evaluate(
        ...
        **kwargs,
        ...
        for i, rocket in enumerate(rockets):
            ...
            f_out[i, :] = (np.log10(perf.cost), -np.log10(max(1.0, perf.payload_mass)))
            if lc:
                g_out[i, :] = (
                    perf.delta_structural,
                    perf.delta_payload,
                    perf.delta_delta_v,
                )
            else:
            ...
            for i_stage in range(n_stages):
                ...
                if lc:
                    if i_stage == 0:
                        ...
                        stage_length = (
                            stage_length * (stage1_bounds[1] - stage1_bounds[0])
                            + stage1_bounds[0]
                        )
                    elif i_stage > 0:
                ...
                stages.append(
                    Stage(engines=[engine_type] * n_engines, length=stage_length)
                )
    ...
    def _correct_x(self, x: np.ndarray, is_active: np.ndarray):
        for n_stages in [1, 2, 3]:
            ...
            is_active[i_x_n_stages, 1 + 2 * n_stages : 7] = (
                False  # Engine selection inactiveness
            )
            is_active[i_x_n_stages, 7 + n_stages : 10] = (
                False  # Stage length inactiveness
            )
    ...
    def _gen_all_discrete_x(self) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        ...
        for n_stages in range(3):
            ...
            x_engines = np.array(
                list(itertools.product(*(opts_engines * (n_stages + 1)))), dtype=int
            )
        ...
        x_all[:, [11]] = np.tile(
            np.array([np.arange(3)]).T, (x_stages.shape[0], 1)
        )  # Head shape
