...
__all__ = [
    ...
    "RandomHiddenConstraintsBase",
    "HCSphere",
    "HierarchicalRosenbrockHC",
    "ConstraintHiderMetaProblem",
    "CantileveredBeamHC",
    "MDCantileveredBeamHC",
    "CarsideHC",
    "MDCarsideHC",
    "CarsideHCLess",
    "MDMueller02",
        ...
        **kwargs,
        ...
        def _is_failed(ix):
            for i_dv in range(x.shape[1]):
                if -0.2 <= x[ix, i_dv] <= 0.2:
                    return True

            cx = np.sum(x[ix, :] * (np.sin(x[ix, :]) + 0.1))
            return cx > 0
...
class Mueller02(SampledFailureRateMixin, NoHierarchyProblemBase):
    ...
    def _arch_evaluate(
        ...
        **kwargs,
        ...
        cx = (
            np.sum(
                (x * np.sin(x_min_term) * np.cos(x_plus_term))
                + ((x[:, [0]] + 1) * np.sin(x_plus_term) * np.cos(x_min_term)),
                axis=1,
            )
            - 5
        )
...
class HierMueller02(SampledFailureRateMixin, TunableHierarchicalMetaProblem):
    ...
    def __init__(self):
        super().__init__(
            lambda n: Mueller02(), imp_ratio=6.0, n_subproblem=20, diversity_range=0.5
        )
...
class Mueller08(SampledFailureRateMixin, NoHierarchyProblemBase):
    ...
    def _arch_evaluate(
        ...
        **kwargs,
        ...
        if self._mo:
            inner_sum = [
                np.sum(j * np.cos((j + 1) * x[:, :-1]) + j, axis=1) for j in range(1, 6)
            ]
...
class HierMueller08(SampledFailureRateMixin, TunableHierarchicalMetaProblem):
    ...
    def __init__(self):
        super().__init__(
            lambda n: Mueller08(), imp_ratio=6.0, n_subproblem=20, diversity_range=0.5
        )
...
class MOHierMueller08(SampledFailureRateMixin, TunableHierarchicalMetaProblem):
    ...
    def __init__(self):
        super().__init__(
            lambda n: MOMueller08(), imp_ratio=6.0, n_subproblem=20, diversity_range=0.5
        )
    ...
    def _arch_evaluate(
        ...
        **kwargs,
        ...
        cx = (
            ...
            + 0.1
            * np.sum(
                4 * (x_fail - 0.7) ** 2 - 2 * np.cos(4 * np.pi * (x_fail - 0.7)), axis=1
            )
            - 0.25
        )
        f_out[cx >= 0, :] = np.nan
...
class HierAlimo(SampledFailureRateMixin, TunableHierarchicalMetaProblem):
    ...
    def __init__(self):
        super().__init__(
            lambda n: Alimo(), imp_ratio=6.0, n_subproblem=20, diversity_range=0.5
        )
...
class HierAlimoEdge(SampledFailureRateMixin, TunableHierarchicalMetaProblem):
    ...
    def __init__(self):
        super().__init__(
            lambda n: AlimoEdge(), imp_ratio=6.0, n_subproblem=20, diversity_range=0.5
        )
...
class RandomHiddenConstraintsBase(SampledFailureRateMixin, NoHierarchyProblemBase):
    ...
    def __init__(self, des_vars, density=0.25, radius=0.1, seed=None, **kwargs):
    ...
    def _set_failed_points(
        self, x: np.ndarray, f_out: np.ndarray, g_out: np.ndarray, h_out: np.ndarray
    ):
        if self._x_failed is None:
            if self.seed is not None:
                np.random.seed(self.seed)
            x_failed = HierarchicalSampling().do(self, 100).get("X")
            i_selected = np.random.choice(
                len(x_failed), size=int(self.density * len(x_failed)), replace=False
            )
        ...
        is_failed = np.any(
            distance.cdist(x * self._scale, self._x_failed) < self.radius, axis=1
        )
...
class HCSphere(RandomHiddenConstraintsBase):
    ...
    def __init__(self):
        ...
        super().__init__(des_vars, density=self._density, radius=0.1, seed=0)
    ...
    def _arch_evaluate(
        ...
        **kwargs,
        ...
        f_out[:, 0] = np.sum((x - 0.5 * (self.xu - self.xl)) ** 2, axis=1)
...
class MOHierarchicalRosenbrockHC(SampledFailureRateMixin, MOHierarchicalRosenbrock):
    ...
    def _arch_evaluate(
        ...
        **kwargs,
        ...
        hc_violated = g[:, 1] > 0.0
        if self._mo:
            hc_violated |= np.abs(0.5 - (f_out[:, 0] / 20) % 1) > 0.35
            hc_violated |= (f_out[:, 1] > 1000) & (
                np.abs(0.5 - (f_out[:, 1] / 100) % 1) > 0.35
            )
...
class HCMOHierarchicalTestProblem(SampledFailureRateMixin, HierarchicalMetaProblemBase):
    ...
    def __init__(self):
        super().__init__(
            MOHierarchicalRosenbrockHC(), n_rep=2, n_maps=2, f_par_range=[100, 100]
        )
...
class Tfaily02(SampledFailureRateMixin, Branin):
    ...
    def _arch_evaluate(
        ...
        **kwargs,
        ...
        is_failed = (np.abs(x[:, 0] - 0.5) < 0.5) & (np.abs(x[:, 1] - 0.5) < 0.4)
        f_out[is_failed, :] = np.nan
