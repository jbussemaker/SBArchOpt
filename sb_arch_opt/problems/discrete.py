    ...
    def _arch_evaluate(
        ...
        **kwargs
        ...
        for i in range(x.shape[0]):
            ...
            z1, z2 = x[i, 2], x[i, 3]
            if z1 == 0:
                f_out[i, 0] = h if z2 == 0 else (0.4 * h + 1.1)
            else:
                f_out[i, 0] = (-0.75 * h + 5.2) if z2 == 0 else (-0.5 * h - 2.1)

    def plot(self, z1=0, z2=0, show=True):
        xx, yy = np.meshgrid(np.linspace(0, 1, 50), np.linspace(0, 1, 50))
        x = np.column_stack(
            [xx.ravel(), yy.ravel(), np.ones((xx.size,)) * z1, np.ones((xx.size,)) * z2]
        )
...
class AugmentedMDBranin(MDBranin):
    ...
    _des_vars = [
        Real(bounds=(0, 1)) if i < 10 else Choice(options=[0, 1]) for i in range(12)
    ]
    ...
    def _arch_evaluate(
        ...
        **kwargs
        ...
        f = [
            ...
            lambda x_: 2 * np.cos(1.1 * np.pi * np.exp(x_)) - 0.5 * x_ + 2,
            lambda x_: np.cos(2 * np.pi * x_) + 0.5 * x_,
            lambda x_: x_ * (np.cos(3.4 * np.pi * (x_ - 1)) - 0.5 * (x_ - 1)),
            lambda x_: -0.5 * x_**2,
            lambda x_: 2 * np.cos(0.25 * np.pi * np.exp(-(x_**4))) ** 2 - 0.5 * x_ + 1,
            lambda x_: x_ * np.cos(3.4 * np.pi * x_) - 0.5 * x_ + 1,
            lambda x_: x_ * (-np.cos(7 * 0.5 * np.pi * x_) - 0.5 * x_) + 2,
            lambda x_: -0.5 * x_**5 + 1,
            lambda x_: -np.cos(5 * 0.5 * np.pi * x_) ** 2 * np.sqrt(x_)
            - 0.5 * np.log(x_ + 0.5)
            - 1.3,
        ]
