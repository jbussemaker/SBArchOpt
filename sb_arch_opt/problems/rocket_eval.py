...
warnings.filterwarnings(
    "ignore", message="Tolerance of .* reached", category=RuntimeWarning
)
...
def check_dependency():
    if not HAS_ROCKET:
        raise RuntimeError(
            "Rocket dependencies not installed: pip install sb-arch-opt[rocket]"
        )
...
class Rocket:
    ...
    cone_angle: float = 45.0  # deg
    ellipse_l_ratio: float = 0.2
    length_diameter_ratio: float = 12.0
    max_q: float = 50000.0  # Pa
    payload_density: float = 2810.0  # kg/m3
...
class RocketEvaluator:
    ...
    def evaluate(cls, rocket: Rocket) -> RocketPerformance:
        ...
        if sum(stage_lengths) == 0:
            stage_lengths = [0.1 for _ in range(len(stage_lengths))]
        stage_engines = [
            "".join(engine.value.lower() for engine in stage.engines)
        ...
        (
            ...
            fuel_surfaces,
            oxidizer_surfaces,
        ) = cls.calculate_geometry(
            rocket.length_diameter_ratio,
            stage_lengths,
            rocket.head_shape.value,
            rocket.cone_angle,
            rocket.ellipse_l_ratio,
            stage_engines,
        )
        ...
        for i, stage in enumerate(rocket.stages):
            # Engine performance
            is_liquid = True
            thrust, expansion_ratio, mdot = cls.calculate_liquid_engine(
                stage.engines.count(Engine.VULCAIN),
            ...
            if thrust == 0:
                is_liquid = False
                expansion_ratio = 0
                thrust, mdot = cls.calculate_solid_engine(
                    stage.engines.count(Engine.SRB),
            ...
            propellant_mass, h2_mass, lox_mass = cls.calculate_propellant_mass(
                n_srb, n_p80, n_gem60, stages_volume[i], fuel_volumes[i], ox_volumes[i]
            )
            ...
            stage_struct_mass = (
                (pumps_mass + mass_tank + mass_insulation) if is_liquid else mass_casing
            )
        ...
        length_ratio = (
            rocket.ellipse_l_ratio if rocket.head_shape == HeadShape.ELLIPTICAL else 0
        )
        payload_mass, h_vector, v_vector, delta_delta_v = cls.calculate_trajectory(
            ...
            m_payload_fix=rocket.payload_mass,
        )
        ...
        # Constraints
        delta_structural = cls.calculate_max_q_constraint(
            rocket.max_q, h_vector, v_vector
        )
        delta_payload = cls.calculate_payload_constraint(
            volume_available, payload_mass, rocket.payload_density
        )
    ...
    def calculate_liquid_engine(vulcain, rs68, s_ivb):
        ...
        expansion_ratios = {"VULCAIN": 45, "RS68": 21.5, "S_IVB": 28}

        mdot_dic = {
            "VULCAIN": 188.33,  # Mass flow in kg/s
            "RS68": 807.39,
            "S_IVB": 247,
        }

        t_per_engine = {
            "VULCAIN": 0.8e6,  # Thrust in N
            "RS68": 2.891e6,
            "S_IVB": 0.486e6,
        }
        ...
        if vulcain + rs68 + s_ivb > 0:
            thrust = (
                vulcain * t_per_engine["VULCAIN"]
                + rs68 * t_per_engine["RS68"]
                + s_ivb * t_per_engine["S_IVB"]
            )
    ...
    def calculate_solid_engine(srb, p80, gem60):
        ...
        mdot_dic = {"SRB": 5290, "P80": 764, "GEM60": 463.18}  # Mass flow in kg/s

        thrust_per_engine = {
            "SRB": 12.45e6,  # Thrust in N
            "P80": 2.1e6,
            "GEM60": 1.245e6,
        }
        ...
        if srb + p80 + gem60 > 0:
            thrust = (
                srb * thrust_per_engine["SRB"]
                + p80 * thrust_per_engine["P80"]
                + gem60 * thrust_per_engine["GEM60"]
            )
        if srb > 0:
    ...
    def calculate_geometry(
        l_d, length_stages_list, head_shape, cone_angle, l_ratio, engines_stages
    ):
        """Calculation of the launcher geometric properties."""

        ratio_o_f = 7.937  # Stoichiometric oxidizer to fuel ratio
        densities_per_material = {"LOX": 1140, "LH2": 71}
        ...
        if head_shape == "Cone":
            ...
            surface_tip = (
                math.pi * l_ellipse**2
                + (np.log((1 + eps) / (1 - eps)) * math.pi / eps * radius**2) / 2
            )
        ...
        for stage in range(len(length_stages_list)):
            ...
            if vulcain + rs68 + s_ivb > 0:
                volume_h2 = volume_stage / (
                    densities_per_material["LH2"]
                    * ratio_o_f
                    / densities_per_material["LOX"]
                    + 1
                )
                ...
                s_tank_oxidizer = (
                    math.pi * radius**2 * 2 + 2 * math.pi * radius * height_oxidizer
                )
                s_tank_fuel = (
                    math.pi * radius**2 * 2 + 2 * math.pi * radius * height_fuel
                )
    ...
    def calculate_propellant_mass(srb, p80, gem60, volume_tank, volume_h2, volume_lox):
        ...
        densities_per_material = {
            "PBAN": 1715,  # Propellant densities in kg/m^3
            "HTPB1912": 1810,
            "HTPB_APCP": 1650,
            "LOX": 1140,
            "LH2": 71,
        ...
        corrections_per_material = {
            "PBAN": 1,  # Correction for ullage volume
            "HTPB1912": 1,
            "HTPB_APCP": 1,
            "LOX": 0.94,
        }
        ...
        if srb > 0:
            propellant_mass = (
                densities_per_material["PBAN"]
                * corrections_per_material["PBAN"]
                * volume_tank
            )
        elif p80 > 0:
            propellant_mass = (
                densities_per_material["HTPB1912"]
                * corrections_per_material["HTPB1912"]
                * volume_tank
            )
        elif gem60 > 0:
            propellant_mass = (
                densities_per_material["HTPB_APCP"]
                * corrections_per_material["HTPB_APCP"]
                * volume_tank
            )
        else:
            h2_mass = (
    ...
    def calculate_structural_mass(
        ...
        if srb + p80 + gem60 > 0:  # Solid
            ...
            pumps_mass = (
                7.81e-4 * thrust_per_engine + 3.37e-5 * expansion_ratio**0.5 + 59
            ) * n_engines
    ...
    def calculate_trajectory(
        ...
        def simulate_trajectory(m_payload):
            ...
            def stage_state_eq(t_, y):  # Trajectory equation
                ...
                return [
                    v_,
                    T / ((m0 + m_payload) - mdot * t_) * np.cos(alpha)
                    - 1
                    / 2
                    / ((m0 + m_payload) - mdot * t_)
                    * s
                    * cd
                    * cls.modified_atmosphere(x)
                    * v_**2
                    - g * np.sin(gamma),
                ]
            ...
            if (
                len(pos) == 0 and stage < stages_max
            ):  # This means that a second rocket stage is available and needed
                h_first += h.tolist()
                v_first += v.tolist()
                stage = stage + 1
                T = T_stages[stage]
                ...
                if (
                    len(pos) == 0 and stage < stages_max
                ):  # This means that a third rocket stage is available and needed
                    h_first += h.tolist()
                    v_first += v.tolist()
                    stage = stage + 1
                    T = T_stages[stage]
            ...
            m0 = m0 - mdot * t_0
            mp_remaining = (
                mp_stages[stage] - mdot * t_0
            )  # It could be that the previous stage did not finish
            h_second = []
            v_second = []
            ...
            if (
                len(pos2) == 0 and stage < stages_max
            ):  # This means that a second rocket stage is available and needed
                h_second += h.tolist()
                v_second += v.tolist()
                stage = stage + 1
                ...
                if (
                    len(pos2) == 0 and stage < stages_max
                ):  # A third rocket stage is available and needed
                    h_second += h.tolist()
                    v_second += v.tolist()
                    stage = stage + 1
                    mp_remaining = mp_stages[stage]
            ...
            if m0 + m_payload - mp_remaining <= 0:
                ...
                delta_v = (
                    T
                    / mdot
                    * np.log((m0 + m_payload) / (m0 + m_payload - mp_remaining))
                )
    ...
    def calculate_cost(
        ...
        # Fuel costs
        fuel_cost_per_kg = {"Solid": 5, "LOX": 0.27, "LH2": 6.1}
        ...
        for index in range(len(n_engines_list)):
            ...
            # Engine cost
            if m_solid > 0:  # Solid
                production_cost_engines_stage = (
                    0.85 * n_engines * 2.3 * (m_engine + m_solid) ** 0.399
                )  # page 125 TC
            else:  # Liquid
                production_cost_engines_stage = (
                    0.85 * n_engines * 5.16 * m_engine**0.45
                )  # page 129 TRANSCOST
            ...
            if m_h2 > 0:
                fuel_cost_stage = (
                    m_h2 * fuel_cost_per_kg["LH2"] + m_lox * fuel_cost_per_kg["LOX"]
                )
            else:
            ...
            total_cost = (
                total_cost + engine_cost_stage + fuel_cost_stage + structure_cost_stage
            )
