...
def test_init_no_vars():
    ...
    assert ds.discrete_imputation_ratio == 1.0
    assert ds.continuous_imputation_ratio == 1
    assert ds.imputation_ratio == 1
    assert ds.discrete_correction_ratio == 1.0
...
def test_num_x():
    ...
    assert np.all(np.abs(np.min(x, axis=0) - ds.xl) < 0.1)
    assert np.all(np.abs(np.max(x, axis=0) - ds.xu) < 0.1)
    assert is_active.shape == x.shape
    assert np.all(is_active)
...
def test_forbidden():
    ...
    ds.add_forbidden_clause(
        ForbiddenAndConjunction(  # Forbid b == F iff a == C
            ForbiddenEqualsClause(ds["a"], "C"),
            ForbiddenEqualsClause(ds["b"], "F"),
        )
    )
...
def test_add_value_constraint():
    for src_as_list, tgt_as_list in [
        (False, False),
        (True, False),
        (False, True),
        (True, True),
        ...
        ds.add_value_constraint(
            ds["b"],
            ["F"] if tgt_as_list else "F",
            ds["a"],
            ["C"] if src_as_list else "C",
        )
