...
def test_arch_sbo_rbf(problem: ArchOptProblemBase):
    ...
    result = minimize(
        problem, sbo, termination=("n_eval", 12), verbose=True, progress=True
    )
...
def test_arch_sbo_rbf_seed(problem: ArchOptProblemBase):
    result = minimize(
        problem, get_arch_sbo_rbf(init_size=10), termination=("n_eval", 12), seed=42
    )
    x1 = result.pop.get("X")
...
def test_arch_sbo_rbf_failing(failing_problem: ArchOptProblemBase):
    ...
    sbo = get_arch_sbo_rbf(init_size=10)
    result = minimize(
        failing_problem, sbo, termination=("n_eval", 12), verbose=True, progress=True
    )
    assert len(result.pop) == 12
...
def test_arch_sbo_gp_seed(problem: ArchOptProblemBase):
    result = minimize(
        problem,
        get_arch_sbo_gp(problem, init_size=10),
        termination=("n_eval", 11),
        seed=42,
    )
    x1 = result.pop.get("X")
...
def test_constraint_handling():
    ...
    for strategy, n_g_infill in [
        ...
        (
            MeanConstraintPrediction(aggregation=ConstraintAggregation.ELIMINATE),
            set(range(1, 5)),
        ),
        (MeanConstraintPrediction(aggregation=ConstraintAggregation.AGGREGATE), {1}),
        (ProbabilityOfFeasibility(), {problem.n_ieq_constr}),
        (ProbabilityOfFeasibility(min_pof=0.25), {problem.n_ieq_constr}),
    ]:
        model = ModelFactory.get_kriging_model()
        ...
        result = minimize(
            problem, sbo, termination=("n_eval", 12), copy_algorithm=False
        )
...
def test_invalid_training_set(problem: ArchOptProblemBase):
    ...
    sbo = FailedXYRemovingSBO(
        RBF(print_global=False),
        FunctionEstimateInfill(),
        pop_size=100,
        termination=100,
        repair=ArchOptRepair(),
    ).algorithm(infill_size=1, init_size=10)
    sbo.setup(problem)
...
def test_smt_krg_features():
    ...
    def _try_model(
        ...
        if cont_relax and norm_ds_spec.is_mixed_discrete:
            cr_ds_spec = model_factory.create_smt_design_space_spec(
                ds, md_normalize=True, cont_relax=True
            )
            ...
            model_ds = model_factory.create_smt_design_space_spec(
                ds, md_normalize=True, ignore_hierarchy=True
            ).design_space
        else:
