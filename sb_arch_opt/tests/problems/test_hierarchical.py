...
def run_test_hierarchy(
    problem,
    imp_ratio,
    check_n_valid=True,
    validate_exhaustive=False,
    exh_n_cont=3,
    corr_ratio=None,
):
    ...
    if check_n_valid or x_discrete is not None:
        if x_discrete is not None:
            ...
            if validate_exhaustive:
                x_trail_repair, _ = (
                    HierarchicalExhaustiveSampling.get_all_x_discrete_by_trial_and_repair(
                        problem
                    )
                )
                assert {tuple(ix) for ix in x_trail_repair} == {
                    tuple(ix) for ix in x_discrete
                }
    ...
    x_discrete, is_act_discrete = problem.all_discrete_x
    assert HierarchicalExhaustiveSampling.has_cheap_all_x_discrete(problem) == (
        x_discrete is not None
    )

    assert problem.get_discrete_imputation_ratio() == pytest.approx(imp_ratio, rel=0.02)
    ...
    if np.isnan(corr_ratio):
        ...
        assert problem.get_discrete_correction_ratio() == pytest.approx(
            corr_ratio, rel=0.02
        )
        assert (
            problem.get_discrete_correction_ratio()
            <= problem.get_discrete_imputation_ratio()
        )
    problem.print_stats()
    ...
    if (
        exh_n_cont != -1
        and HierarchicalExhaustiveSampling.get_n_sample_exhaustive(
...
def test_hier_zaefferer():
    run_test_hierarchy(
        ZaeffererHierarchical.from_mode(ZaeffererProblemMode.A_OPT_INACT_IMP_PROF_UNI),
        1,
    )

    problem = ZaeffererHierarchical.from_mode(
        ZaeffererProblemMode.A_OPT_INACT_IMP_PROF_UNI
    )
    x, is_act = problem.correct_x(np.array([[0, 0.75]]))
...
def test_tunable_hierarchical_meta_problem():
    prob1 = TunableHierarchicalMetaProblem(
        lambda n: Branin(), imp_ratio=10, n_subproblem=4, diversity_range=0.5
    )
    prob1.print_stats()

    prob2 = TunableHierarchicalMetaProblem(
        lambda n: Branin(), imp_ratio=10, n_subproblem=5, diversity_range=0.25
    )
    prob2.print_stats()
