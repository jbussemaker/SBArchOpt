...
class DummyProblem(ArchOptTestProblemBase):
    ...
    def __init__(self, only_discrete=False, fail=False):
        ...
        if only_discrete:
            des_vars = [
                (
                    Choice(options=[str(9 - j) for j in range(10)])
                    if i == 0
                    else Integer(bounds=(1, 10))
                )
    ...
    def _gen_all_discrete_x(self) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        if not self._provide_all_x:
            return
        x, is_active = [], []
        cartesian_prod_values = (
            HierarchicalExhaustiveSampling.get_exhaustive_sample_values(self, n_cont=1)
        )
        if self.only_discrete:
    ...
    def _arch_evaluate(
        ...
        **kwargs,
        ...
        x_eval = x.copy()
        x_eval[:, self.is_discrete_mask] = (
            x_eval[:, self.is_discrete_mask] - self.xl[self.is_discrete_mask]
        ) / 9
